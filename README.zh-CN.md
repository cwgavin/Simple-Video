...
- **裁剪视频**：可视化调整裁剪区域，支持拖动边缘/角点、固定比例预设、自动检测黑边，以及带预览的起止时间范围
- **裁剪音频**：支持预览、设置起止时间范围，并可按播放倍率导出
- **转换视频**：在常见视频格式和编码之间转换（mp4、mov、mkv、webm、avi、flv、m4v、ts）
- **转换音频**：在常见音频格式之间转换（mp3、aac、wav、flac、ogg、m4a、wma、aiff、opus）
- **合并音视频**：把单独的视频轨和音频轨合并成一个文件
- **拼接文件**：拼接多个视频或音频文件
- **按时间戳分割**：根据时间戳列表把一个视频切成多个片段
- **移除时间段**：移除两个时间戳之间的片段，并把剩余部分合并成一个新视频
- **语音转文字**：使用 Whisper 将音频/视频转写成文本
- 支持 English 和简体中文语言设置，并在设置页提供第三方许可链接
